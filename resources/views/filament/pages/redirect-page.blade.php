        ...
        <div class="flex flex-col w-full">
            <div class="overflow-x-auto w-full">
                <form method="GET" action="{{ route('filament.admin.pages.redirect-page') }}" class="mb-4 space-y-4 px-4">
                    ...
                    <div style="margin-left: 10px; margin-top: 30px; font-weight: bold;">
                        Фильтры
                    </div>
                    <div class="flex flex-wrap items-center space-y-4 sm:space-y-0 sm:space-x-4">
                        ...
                        <div class="items-center" style="margin: 10px">
                            <label class="text-gray-700 font-medium">Гео:</label>
                                    ...
                                    <div class="p-2 space-y-2">
                                        @foreach ($allGeos as $geo)
                                            <label class="inline-flex items-center">
                                                ...
                                                <span class="text-sm" style="margin-left: 5px">{{ $geo }}</span>
                                            </label>
                                        @endforeach
                                    </div>
                                </div>
                        ...
                        <div class="items-center" style="margin: 10px">
                            <label class="text-gray-700 font-medium">Блоггеры:</label>
                            <div x-data="{ open: false, selectedBloggers: {{ json_encode(request('blogger_ids', [])) }} }" class="relative">
                                <button type="button" @click="open = !open" class="form-input block w-40 pl-3 pr-10 py-2 text-sm font-medium border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                                    Блоггеры
                                    <svg x-bind:class="{ 'transform rotate-180': open, 'ml-2 h-4 w-4': true }" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
                                ...
                                <div x-show="open" @click.away="open = false" class="absolute mt-2 w-40 rounded-md shadow-lg bg-white ring-1 ring-black ring-opacity-5 overflow-hidden z-10">
                                    <div class="p-2 space-y-2">
                                        @foreach ($allBloggers as $key => $blogger)
                                            <label class="inline-flex items-center">
                                                <input type="checkbox" name="blogger_ids[]" x-model="selectedBloggers" value="{{ $key }}" class="form-checkbox" :checked="selectedBloggers.includes('{{ $key }}')">
                                                <span class="text-sm" style="margin-left: 5px">{{ $blogger }}</span>
                                            </label>
                                        @endforeach
                                    </div>
                                </div>
                        ...
                        <div class="items-center" style="margin: 10px">
                            <label class="text-gray-700 font-medium">Ссылки:</label>
                            <div x-data="{ open: false, selectedLinks: {{ json_encode(request('link_ids', [])) }} }" class="relative">
                                <button type="button" @click="open = !open" class="form-input block w-40 pl-3 pr-10 py-2 text-sm font-medium border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                                    Ссылки
                                    <svg x-bind:class="{ 'transform rotate-180': open, 'ml-2 h-4 w-4': true }" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
                                        <path fill-rule="evenodd" d="M10 12a2 2 0 100-4 2 2 0 000 4z" />
                                ...
                                <div x-show="open" @click.away="open = false" class="absolute mt-2 w-40 rounded-md shadow-lg bg-white ring-1 ring-black ring-opacity-5 overflow-hidden z-10">
                                    <div class="p-2 space-y-2">
                                        @foreach ($allLinks as $key => $link)
                                            <label class="inline-flex items-center">
                                                <input type="checkbox" name="link_ids[]" x-model="selectedLinks" value="{{ $key }}" class="form-checkbox" :checked="selectedLinks.includes('{{ $key }}')">
                                                <span class="text-sm" style="margin-left: 5px">{{ $link }}</span>
                                            </label>
                                        @endforeach
                                    </div>
                                </div>
                        ...
                        <div class="items-center" style="margin: 10px">
                            <label class="text-gray-700 font-medium">Ссылки:</label>
                            <div x-data="{ open: false, selectedGeneratedLinks: {{ json_encode(request('generate_link_ids', [])) }} }" class="relative">
                                <button type="button" @click="open = !open" class="form-input block w-40 pl-3 pr-10 py-2 text-sm font-medium border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                                ...
                                <div x-show="open" @click.away="open = false" class="absolute mt-2 w-40 rounded-md shadow-lg bg-white ring-1 ring-black ring-opacity-5 overflow-hidden z-10">
                                    <div class="p-2 space-y-2">
                                        @foreach ($allGenerateLinks as $key => $link)
                                            <label class="inline-flex items-center">
                                                <input type="checkbox" name="generate_link_ids[]" x-model="selectedGeneratedLinks" value="{{ $key }}" class="form-checkbox" :checked="selectedGeneratedLinks.includes('{{ $key }}')">
                                                <span class="text-sm" style="margin-left: 5px">{{ $link }}</span>
                                            </label>
                                        @endforeach
                                    </div>
                ...
                <div class="flex justify-end mb-4">
                    <form method="GET" action="{{ route('filament.admin.pages.export-csv') }}">
                        ...
                        <input type="hidden" name="blogger_ids" value="{{ json_encode(request('blogger_ids', [])) }}">
                        <input type="hidden" name="link_ids" value="{{ json_encode(request('link_ids', [])) }}">
                        <input type="hidden" name="generate_link_ids" value="{{ json_encode(request('generate_link_ids', [])) }}">
